...
int main()
{
	...
	//for (auto it = pair.rbegin(); it != pair.rend(); ++it)
	//{
...
//	}
	...
	try
	{
		...
		for (auto it = distribution.rbegin(); it != distribution.rend(); ++it)
		{
			double object_color[3];

			object_color[0] = it->second->get_color();
			object_color[1] = it->second->get_color();
			object_color[2] = it->second->get_color();
